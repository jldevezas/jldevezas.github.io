...
JldVisualization.prototype.makeWeeklyAnalysisTable = function(limit) {
	...
	jld.weeklyAnalysisData.forEach(function(row) {
		...
		$(jld.containers.preferredweekdays + " tbody")
			.append($("<tr>").addClass(prefInfo == "None" ? "fade-text" : "")
		...
		$(jld.containers.avoidedweekdays + " tbody")
			.append($("<tr>").addClass(avoiInfo == "None" ? "fade-text" : "")
				.append($("<td>").text(row["artist"]))
	...
	var barContainer = svg.selectAll(".bar-week")
		...
		.enter().append("g")
			.on("mouseover", function(d, i) {
				...
				var m = d3.mouse(d3.select(jld.containers.topartistweeklychart).node());
				d3.select(jld.containers.topartistweeklytooltip)
	...
	svg.selectAll(".bar-week")
		...
		.enter().append("rect")
			...
			.on("mouseover", function(d, i) {
				...
				var m = d3.mouse(d3.select(jld.containers.artistweeklychart).node());
				d3.select(jld.containers.artistweeklytooltip)
					.style("left", (m[0] + 25) + "px")
					.style("top", (m[1] + 35) + "px")
					.text(d.weekday + ": " + d.plays);
